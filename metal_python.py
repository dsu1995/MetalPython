...
import tank_bullet
import tank_shell
import title_animation
import title_screen_button

from sprites import enemy_bullet, pistol_bullet, score_board
...
def instructions():
    ...
    button = title_screen_button.TitleScreenButton(3)
...
def title_screen():
    ...
    animations = [title_animation.TitleAnimation(i) for i in range(2)]
    buttons = [title_screen_button.TitleScreenButton(i) for i in range(3)]
...
def level1():
    ...
    bkgd = background.Background(player)

    #     map objects    
    wall = platform.Platform(((1438, 380), (1, 100)))
    platforms = pygame.sprite.Group([platform.Platform(dimension) for dimension in (
    ...
    #     scoreboard
    scoreboard = score_board.ScoreBoard(player, tank)

    #     enemies
    enemiesGrp = pygame.sprite.Group([enemy.Enemy(midbottom) for midbottom in (
    ...
    while keepGoing:
                    ...
                    if event.key == pygame.K_e:
                        ...
                        if current_player == tank:
                            if tank.shoot_cannon():
                                grenadeGrp.add(tank_shell.TankShell(tank))
                                allSprites.add(grenadeGrp)
                        # throw grenade
                        elif player.get_grenades():
                            player.throw_grenade()
                            grenadeGrp.add(grenade.Grenade(player))
                            allSprites.add(grenadeGrp)
        ...
        if not current_player.get_dying():
            ...
            if current_player == tank:
                ...
                if keys_pressed[pygame.K_k]:
                    tank.shoot_mg()
                    pBulletsGrp.add(tank_bullet.TankBullet(bkgd, tank))
            ...
            # player control
                ...
                if keys_pressed[pygame.K_k]:
                    ...
                    if player.get_weapon():
                        pBulletsGrp.add(mg_bullet.MGBullet(bkgd, player, player.shoot()))
                        allSprites.add(pBulletsGrp)
                    # shoot pistol
                    elif player.shoot():
                        pBulletsGrp.add(pistol_bullet.PistolBullet(bkgd, player))
                        allSprites.add(pBulletsGrp)
        # collision detection
        ...
        for bullet, enemy in pygame.sprite.groupcollide(pBulletsGrp, enemiesGrp, False, False).items():
            if enemy and not enemy[0].get_dying():
                bullet.kill()
                enemy[0].die()

                # grenade collision with enemies
        for grenade, enemy in pygame.sprite.groupcollide(grenadeGrp, enemiesGrp, False, True).items():
            if enemy:
                grenade.explode()
                for i in enemy:
                    i.die()
        ...
        for grenade, platform in pygame.sprite.groupcollide(grenadeGrp, platforms, False, False).items():
            if platform:
                grenade.explode()
        ...
        for enemy in enemiesGrp:
            if enemy.get_shooting():
                eBulletsGrp.add(enemy_bullet.EnemyBullet(enemy, current_player))
...
def gameover():
    # Entities
    bkgd = game_over_screen.GameOverScreen()
...
def boss(prev_player, prev_tank=None):
    # Entities
    # player
    player = player.Player(prev_player)
    if prev_tank:
        tank = tank.Tank(prev_tank)
        current_player = tank
    ...
    bkgd = background.Background(player, 1)

    #     map objects    
    wall = platform.Platform(((865, 0), (1, 480)))
    platform = platform.Platform(((0, 432), (1280, 1)))
    ...
    while keepGoing:
            ...
            if not cutscene and not current_player.get_dying():
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_e:
                        ...
                        if current_player == tank:
                            if tank.shoot_cannon():
                                pGrenadeGrp.add(tank_shell.TankShell(tank))
                                allSprites.add(pGrenadeGrp)
                        # throw grenade
                        elif player.get_grenades():
                            player.throw_grenade()
                            pGrenadeGrp.add(grenade.Grenade(player))
                            allSprites.add(pGrenadeGrp)
        ...
        if boss.get_attack() == 1:
            shellGrp.add(tank_shell.TankShell())
